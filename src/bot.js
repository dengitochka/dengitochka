...
bot.start(async (ctx, next) => {
  ...
  const keyboard = [
    [{ text: 'Узнать кредитный рейтинг', web_app: {url: config.WEB_APP_URL}}], 
    [{ text: 'Что тут можно делать', callback_data: "HelpWebApp"}]
  ]

  ctx.reply(`Привет! Наш бот поможет тебе узнать твой кредитный рейтинг. И это совершенно бесплатно и занимает не больше 5 минут.`, getInlineKeyboard({ keyboard }))
...
async function onNewCommand(ctx, next) {
  ...
  if (command === 'HelpWebApp') {
    ctx.sendMessage('Основной возможностью бота является предоставление кредитного рейтинга для вас. \n\n'
    + 'Также доступна подписка на уведомления об изменении вашего кредитного рейтинга, это поможет вам управлять своей финансовой репутацией. Больше подробностей в нашем блоге.')
  }
